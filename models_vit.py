...
"""model = vit_large_patch16_yo(img_size=256, num_classes=1, global_pool=True)
...
# Example dummy import or definition of your ViT model
# from your_model_file import vit_large_patch16_yo  # <-- Real import here
...
def measure_cpu_inference(model, input_tensor):


    # We'll define a small "wrapper" function that memory_profiler can run
    # while sampling memory usage.
    def _wrapper():
        nonlocal results
        results = run_inference(model, input_tensor)

    # This variable will hold (output, inference_time) after the run
    results = None

    # memory_usage runs _wrapper and samples memory usage at intervals
    # By default, memory_usage returns a list of memory measurements in MB
    mem_usage_list = memory_usage(_wrapper, interval=0.1, max_iterations=1)
    peak_mem_mb = max(mem_usage_list)

    # Unpack the results
    output, inference_time = results

    return inference_time, peak_mem_mb, output


if __name__ == "__main__":
    # ------------------------------------------------------------------
    # 1. Load / define your model on CPU
    # ------------------------------------------------------------------
    model = vit_large_patch16_yo(img_size=256, num_classes=1, global_pool=True)
    model.to("cpu")
    ...
    weight_path = "/Users/niccolo/Documents/UCL/TANGERINE/weights/checkpoint-399.pth"
    checkpoint = torch.load(weight_path, map_location="cpu")
    checkpoint_model = checkpoint["model"] if "model" in checkpoint else checkpoint["model_state"]
    model.load_state_dict(checkpoint_model, strict=False)

    # ------------------------------------------------------------------
    # 3. Prepare an example input tensor
    #    (size: 1 x 1 x 256 x 256 x 256)
    # ------------------------------------------------------------------
    input_tensor = torch.randn(1, 1, 256, 256, 256)

    # ------------------------------------------------------------------
    # 4. Run inference and capture time + peak memory usage
    # ------------------------------------------------------------------
    inference_time, peak_mem_mb, output = measure_cpu_inference(model, input_tensor)

    # ------------------------------------------------------------------
    # 5. Print results
    # ------------------------------------------------------------------
    print(f"CPU Inference Time: {inference_time:.2f} seconds")
    print(f"Peak RAM Usage During Inference: {peak_mem_mb:.2f} MB")
...
# Load the .pth file
model_weights = torch.load('/Users/niccolo/Downloads/demo_data/model_cancer_risk.pth')

# Inspect the keys in the state dictionary
print(model_weights.keys())


# Iterate over each parameter and check if it's trainable
...
# Check a specific layer weight
layer_name = "encoder_img.blocks.0.attn.q.weight"  # Example layer
if layer_name in model_weights:
    print(model_weights[layer_name])  # Print the tensor values
else:
    print(f"Layer {layer_name} not found in weights.")

...
from engine_finetune import evaluate, train_one_epoch_orig
...
def get_args_parser():
    ...
    parser.add_argument('--binary_class_weights', default=None, type=int,  # [x]
                        help='flag binary classification')
...
######## ######## ######## ######## ######## ######## ######## ######## ######## ######## ######## ########
    ...
    if True:  # args.distributed:
        ...
        sampler_train = torch.utils.data.DistributedSampler(
            dataset_train, num_replicas=num_tasks, rank=global_rank, shuffle=True #True
        )
...
######## ######## ######## ######## ######## ######## ######## ########
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_orig(
        ...
        if val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(args.output_dir, 'best_model_loss.pth')
            torch.save(model.state_dict(), best_model_path)
            print(f"New best model saved with validation loss: {best_val_loss:.4f}")

        if log_writer is not None:
            log_writer.add_scalar('perf/val_loss', val_loss, epoch)
            log_writer.add_scalar('perf/val_auc', val_auc, epoch)
            log_writer.add_scalar('perf/val_accuracy', val_accuracy, epoch)
